...
def test() :
    ...
    for la in range(0,3) :
        ...
        print(f"Initialisation à la ligne {la}")

        for cb in range(0,3) :

            wfunc, reg, simulator = circ_init()

            print(f"Initialisation à la colonne {cb}")

            for i in range(0,3) :
                wfunc, bit = process(la, i, circModA, reg, wfunc, simulator)
                print(f"Pour la case {la}, {i}, Alice obtient : {bit}")
            
            for i in range(0,3):
                wfunc, bit = process(i, cb, circModB, reg, wfunc, simulator)
                print(f"Pour la case {i}, {cb}, Bob obtient {bit}")


def test2(lina : int, colb : int) :
    wfunc, reg, simulator = circ_init()

    print(f"Début des tests de calculs aléatoires")

    # 1 2 2 3 1 3 1 3 2
    wfunc, bit = process(lina, 0, circModA, reg, wfunc, simulator)
    print(f"Pour la case {lina}, 1, Alice obtient {bit}")
    
    wfunc, bit = process(0, colb, circModB, reg, wfunc, simulator)
    print(f"Pour la case 1, {colb}, Bob obtient {bit}")

    wfunc, bit = process(lina, 1, circModA, reg, wfunc, simulator)
    print(f"Pour la case {lina}, 2, Alice obtient {bit}")
    
    wfunc, bit = process(1, colb, circModB, reg, wfunc, simulator)
    print(f"Pour la case 2, {colb}, Bob obtient {bit}")
    ...
    wfunc, bit = process(lina, 2, circModA, reg, wfunc, simulator)
    print(f"Pour la case {lina}, 3, Alice obtient {bit}")
    
    wfunc, bit = process(2, colb, circModB, reg, wfunc, simulator)
    print(f"Pour la case 3, {colb}, Bob obtient {bit}")
